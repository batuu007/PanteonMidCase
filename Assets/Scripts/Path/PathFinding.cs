...
public class PathFinding : MonoBehaviour
{
    ...
    IEnumerator FindPath(Vector3 startPos, Vector3 targetPos)
    {
        ...
        PathNode startNode = _gridSystem.NodeFromWorldPoint(startPos);
        PathNode targetNode = _gridSystem.NodeFromWorldPoint(targetPos);
        if (targetNode.isWalkable)
        {


            Stack<PathNode> openList = new Stack<PathNode>(_gridSystem.MaxSize);
            ...
            while (openList.Count > 0)
            {
                ...
                foreach (PathNode item in _gridSystem.GetNeibours(currentNode))
                {
